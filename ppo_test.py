...
def main():
    ms3_vec_env = gym.make("PushCube-v1", obs_mode="rgb+depth", num_envs=64)
    max_episode_steps = gym_utils.find_max_episode_steps_value(ms3_vec_env)
    vec_env = ManiSkillSB3VectorEnv(ms3_vec_env)

    model = PPO("MlpPolicy", vec_env, gamma=0.8, gae_lambda=0.9, n_steps=50, batch_size=128, n_epochs=8, verbose=2)
    ...
    eval_vec_env = gym.make("PushCube-v1", obs_mode="rgb+depth", num_envs=16, render_mode="rgb_array")
