...
class PickCubeRMA(PickCubeEnv):
    ...
    def __init__(self,
        ...
        if self.randomized_env:
            ...
            self.mass_l_scdl = linear_schedule(0.064, 0.01, init_step,
                                               end_step)
        ...
        if self.obs_noise:
            ...
            self.proprio_l_scdl = linear_schedule(0, -.005, init_step,
                                                  end_step)
    ...
    def _get_obs_state_dict(self):
        ...
        if self.obs_noise:
            ...
            proprio_noise = np.random.uniform(self.proprio_l, self.proprio_h,
                                              9)
...
class PickCube(PickCubeEnv):
    ...
    def _initialize_agent(self):
        if self.robot_uid == "panda":
            ...
            # fmt: on
            qpos[:-2] += self._episode_rng.normal(0,
    ...
    def _get_obs_state_dict(self):
        ...
        return OrderedDict(
            observation=flatten_state_dict(
                OrderedDict(proprio=self.agent.get_proprioception(),
                            tcp_pose=vectorize_pose(self.tcp.pose),
                            tcp_to_goal_pos=self.goal_pos - self.tcp.pose.p,
                            obj_pose=vectorize_pose(self.obj.pose),
                            tcp_to_obj_pos=self.obj.pose.p - self.tcp.pose.p,
            ...
            #                             obj_fri=self.obj_friction)),
            goal_info=flatten_state_dict(
                OrderedDict(target_pos=self.goal_pos)).astype("float32"),
        )
