...
class PickSingleRMA(PickSingleEnv):
    ...
    def set_randomization(self):
        ...
        if self.ext_disturbance:
            self.force_scale_scdl = linear_schedule(0., self.force_scale_h,
                                                    init_step, end_step)
        if self.randomized_env:
            self.scale_h_scdl = linear_schedule(1., self.scl_scdl_h, init_step,
                                                end_step)
            self.scale_l_scdl = linear_schedule(1., self.scl_scdl_l, init_step,
                                                end_step)
            self.dens_h_scdl = linear_schedule(1., self.dens_scdl_h, init_step,
                                               end_step)
            self.dens_l_scdl = linear_schedule(1., self.dens_scdl_l, init_step,
                                               end_step)
            self.fric_h_scdl = linear_schedule(1., self.fric_scdl_h, init_step,
                                               end_step)
            self.fric_l_scdl = linear_schedule(1., self.fric_scdl_l, init_step,
                                               end_step)
        ...
        if self.obs_noise:
            ...
            self.proprio_l_scdl = linear_schedule(0, self.prop_scdl_l,
                                                  init_step, end_step)
            self.pos_h_scdl = linear_schedule(0, self.pos_scdl_h, init_step,
                                              end_step)
            self.pos_l_scdl = linear_schedule(0, self.pos_scdl_l, init_step,
                                              end_step)
    ...
    def _get_obs_state_dict(self):
        ...
        if self.ext_disturbance:
            ...
            if grasped:
                self.obj.add_force_at_point(self.disturb_force,
                                            self.obj.pose.p)

        contacts = self._scene.get_contacts()
        limpulse = np.linalg.norm(get_pairwise_contact_impulse(
            contacts, self.agent.finger1_link, self.obj),
                                  ord=2)
        rimpulse = np.linalg.norm(get_pairwise_contact_impulse(
            contacts, self.agent.finger2_link, self.obj),
                                  ord=2)
        ...
        return OrderedDict(
            ...
            goal_info=flatten_state_dict(
                OrderedDict(target_pos=self.goal_pos,
                            tcp_to_goal_pos=self.goal_pos - self.tcp.pose.p,
                            obj_to_goal_pos=self.goal_pos -
                            self.obj.pose.p)).astype("float32"),
        )
...
class PickSingleEGADRMAs(PickSingleRMA, PickSingleEGADEnv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        parent_folder = Path(format_path(
            self.DEFAULT_ASSET_ROOT)) / "egad_train_set"
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.auto_dr:
            if self.eval_env and self.randomized_param == 'obj_scale':
                self.model_scale_mult = random.choice(
    ...
    def _initialize_actors(self):
        ...
        if self.auto_dr:
            if self.eval_env and self.randomized_param == 'obj_friction':
                self.obj_friction = random.choice(
